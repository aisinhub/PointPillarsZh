...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar):
        ...
        zero =  torch.zeros_like(pillars)
        pillars_xyz = zero[:, :, :3] + torch.sum(pillars[:, :, :3], dim=1, keepdim=True) / npoints_per_pillar[:, None, None] # (p1 + p2 + ... + pb, num_points, 3)
        # temp_sum = torch.sum(pillars[:, :, 3], dim=1, keepdim=True)
        # temp_npoint = npoints_per_pillar[:, None, None]
        n_dim = pillars.size(1)
        p_dim = pillars.size(0)
        pillars_z = (torch.sum(pillars[:, :, 3], dim=1, keepdim=True).unsqueeze(-1) / npoints_per_pillar[:, None, None]).expand(p_dim, n_dim, 1) # (p1 + p2 + ... + pb, num_points, 3)
        
        pillars_z_ex = pillars_xyz[:,:,0].unsqueeze(-1)
        print('--------------- x_offset_pi_center')
        print(x_offset_pi_center.shape)
        
        print('--------------- pillars_xyz')
        print(pillars_xyz.shape)
        
        print('--------------- pillars_z')
        print(pillars_z.shape)
        
        print('---------------pillars_z_ex')
        print(pillars_z_ex.shape)
        
        # print('---------------temp_sum')
        # print(temp_sum.shape)
        
        # print('---------------temp_npoint')
        # print(temp_npoint.shape)
        # コンキャット
        features = torch.cat([pillars, offset_pt_center, x_offset_pi_center, y_offset_pi_center, pillars_z], dim=-1) # (p1 + p2 + ... + pb, num_points, 10)
        # features = torch.cat([pillars, offset_pt_center, x_offset_pi_center, y_offset_pi_center], dim=-1) # (p1 + p2 + ... + pb, num_points, 10)
...
class PointPillarsAddZFormer(nn.Module):
    def __init__(self,
                 nclasses=3, 

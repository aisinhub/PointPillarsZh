...
# def initでは，  占有メモリ量が0とでる．def forwardから，メモリが専有され始める
# my import ########################
...
class PillarEncoder(nn.Module):
    def __init__(self, voxel_size, point_cloud_range, in_channel, out_channel):
        ...
        # my 新たな畳み込み層とバッチ正規化層を追加
        self.conv_v2 = nn.Conv1d(8, out_channel, 1, bias=False) # my code
        self.bn_v2 = nn.BatchNorm1d(out_channel, eps=1e-3, momentum=0.01)
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar): 
        ...
        ############################ ↓ my code
        num_pillars=pillars.size(0)
        num_points=pillars.size(1)
        
        z_min = torch.min(pillars[:, :, 2:3], dim=1, keepdim=True)[0] # (p1 + p2 + ... + pb, 1, 1)
        z_max = torch.max(pillars[:, :, 2:3], dim=1, keepdim=True)[0] # (p1 + p2 + ... + pb, 1, 1)
        z_bins = torch.linspace(0, 1, steps=9, device=device).view(1, 1, 9) # (1, 1, 9)
        z_bins = z_min + (z_max - z_min) * z_bins # (p1 + p2 + ... + pb, 1, 9)
        
        z_hist = torch.zeros((pillars.size(0), 8), device=device) # (p1 + p2 + ... + pb, 8)
        
        # ゼロパディングのカウントとゼロパディング部を-100にする
        temp_pillars = pillars
        zero_mask = (temp_pillars[:, :, 0:3] == 0).all(dim=-1) #0パディングされた点の調査
        zero_points_count = zero_mask.sum(dim=1, keepdim=True)  # (P, 1) 各ピラーで0パディングされた点の個数

        temp_pillars[:,:,2][zero_mask] = -100 # 0パディング点のzを-100にする
        
        z_values = temp_pillars[:, :, 2:3]  # (p1 + p2 + ... + pb, num_points, 1)
        z_bins_expanded = z_bins.unsqueeze(2)  # (p1 + p2 + ... + pb, 1, 1, 9)
        z_values_expanded = z_values.unsqueeze(-1)  # (p1 + p2 + ... + pb, num_points, 1, 1)

        # z_valuesが各binに入るかどうかのマスクを計算
        z_mask = (z_values_expanded >= z_bins_expanded[:, :, :, :-1]) & (z_values_expanded < z_bins_expanded[:, :, :, 1:])  # (p1 + p2 + ... + pb, num_points, 1, 8)

        # 各binに含まれる点数を計算
        z_hist = z_mask.sum(dim=1).squeeze(2)  # (p1 + p2 + ... + pb, 1，8)
                    
        # Normalize histogram ヒストグラムの正規化
        npoints_tensor = torch.ones((num_pillars, 1), device=device) * num_points 
        npoints_per_pillar_adjusted = npoints_tensor - zero_points_count  # (p1 + p2 + ... + pb, 1)
        
        npoints_per_pillar_adjusted = npoints_per_pillar_adjusted.unsqueeze(-1) # (p1 + p2 + ... + pb, 1，1)
        npoints_per_pillar_adjusted = npoints_per_pillar_adjusted.expand(-1, 1, 8) # (p1 + p2 + ... + pb, 1，8)
        z_hist = z_hist.float() / npoints_per_pillar_adjusted.float()  # (p1 + p2 + ... + pb,1，8)
      
        # z_hist_expanded = z_hist.expand(-1, pillars.size(1), -1)  # (p1 + p2 + ... + pb, num_points, 8) この形状では，con1dが受け付けなかったため
        # z_histの第2，3次元の入れ替え
        z_hist = z_hist.view(z_hist.size(0), z_hist.size(2), 1) # (p1 + p2 + ... + pb，8，1)
        # z_histを第3次元に沿って num_points に拡張
        z_hist_expand = z_hist.expand(-1, -1, num_points) # (p1 + p2 + ... + pb, 8, num_points)

        # 64次元へ拡張
        z_hist_features = F.relu(self.bn_v2(self.conv_v2(z_hist_expand)))  # (p1 + p2 + ... + pb, out_channels, num_points) (P,64,N)
        # z_hist_features = F.relu(self.bn(self.conv_v2(z_hist_expand)))  # (p1 + p2 + ... + pb, out_channels, num_points) (P,64,N)
        pooling_z_hist_features = torch.max(z_hist_features, dim=-1)[0] # (p1 + p2 + ... + pb, out_channels)
        ...
        for i in range(bs):
            ...
            cur_features_z_hist = pooling_z_hist_features[cur_coors_idx]
            canvas_z_hist = torch.zeros((self.x_l, self.y_l, self.out_channel), dtype=torch.float32, device=device)
            canvas_z_hist[cur_coors[:, 1], cur_coors[:, 2]] = cur_features_z_hist
            canvas_z_hist = canvas_z_hist.permute(2, 1, 0).contiguous()
            batched_z_hist_canvas.append(canvas_z_hist)
                        
        batched_canvas = torch.stack(batched_canvas, dim=0) # (bs, in_channel, self.y_l, self.x_l)
        batched_z_hist_canvas = torch.stack(batched_z_hist_canvas, dim=0) # (bs, in_channel, self.y_l, self.x_l)

        return batched_canvas, batched_z_hist_canvas
...
class Backbone(nn.Module):
    ...
    # @profile # メモリ計測部分 class Backbone Poinstpillars_late_fusion_z_hist
    def forward(self, x):
        '''
...
class Backbone_v2(nn.Module):
    ...
    # @profile # メモリ計測部分 class Backbone_v2 Poinstpillars_late_fusion_z_hist
    def forward(self, x):
        '''
        x: (b, c, y_l, x_l). Default: (6, 64, 496, 432)
        return: list[]. Default: [(6, 64, 248, 216), (6, 128, 124, 108), (6, 256, 62, 54)]
...
class Neck_v2(nn.Module):
    ...
    # @profile # メモリ計測部分 class Neck_V2 Poinstpillars_late_fusion_z_his
    def forward(self, x):
        '''
        x: [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        return: (bs, 384, 248, 216)
...
class Combine(nn.Module):
    def __init__(self):
        super().__init__()
    # @profile # メモリ計測部分 class Combline Poinstpillars_late_fusion_z_his
    def forward(self, x, y):
        '''
        x: (b, C, y_l/2, x_l/2)
        y: (b, C, y_l/2, x_l/2)
        return: (b, 2C, y_l/2, x_l/2)
        '''
    ...
    def __init__(self, in_channel, n_anchors, n_classes):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                conv_layer_id += 1
    # @profile # メモリ計測部分 class Head Poinstpillars_late_fusion_z_his
    def forward(self, x):
        '''
        x: (bs, 384, 248, 216)
        return: 
...
class PointPillarsLateFusionZHist(nn.Module):
    def __init__(self,
                 nclasses=3, 
        ...
        self.backbone = Backbone(in_channel=64, 
                                 ...
                                 layer_nums=[3, 5, 5])
        self.backbone_v2 = Backbone_v2(in_channel=64, 
                                 out_channels=[64, 128, 256], 
                                 layer_nums=[3, 5, 5])
        self.neck = Neck(in_channels=[64, 128, 256], 
                         upsample_strides=[1, 2, 4], 
                         out_channels=[128, 128, 128])
        self.neck_v2 = Neck_v2(in_channels=[64, 128, 256], 
                         upsample_strides=[1, 2, 4], 
                         out_channels=[128, 128, 128])
        
        self.combine = Combine()
        self.head = Head(in_channel=768, n_anchors=2*nclasses, n_classes=nclasses) # 128*3=384*2=768 
        ...
        self.score_thr = 0.1
        self.max_num = 50
    # @profile # メモリ計測部分 class class PointPillarsLateFusionZHist
    ...
    def forward(self, batched_pts, mode='test', batched_gt_bboxes=None, batched_gt_labels=None):
        ...
        pillar_features, pillar_z_hist_features = self.pillar_encoder(pillars, coors_batch, npoints_per_pillar)

        # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        xs = self.backbone(pillar_features)
        xs_z = self.backbone_v2(pillar_z_hist_features)

        # x: (bs, 384, 248, 216)
        x = self.neck(xs)
        x_z = self.neck_v2(xs_z)

        x_syn = self.combine(x, x_z)
        # bbox_cls_pred: (bs, n_anchors*3, 248, 216) 
        # bbox_pred: (bs, n_anchors*7, 248, 216)
        # bbox_dir_cls_pred: (bs, n_anchors*2, 248, 216)
        bbox_cls_pred, bbox_pred, bbox_dir_cls_pred = self.head(x_syn)

...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar):
        ...
        # rの分散を計算
        pillars_r_var = torch.var(pillars[:, :, 3], dim=1).unsqueeze(-1) # (p1 + p2 + ... + pb, 1)
        
        # 付け足し
        pooling_features = torch.cat([pooling_features, pillars_z_center.unsqueeze(-1), pillars_r_var], dim=-1)
        ...
        for i in range(bs):
            ...
            canvas = torch.zeros((self.x_l, self.y_l, 66), dtype=torch.float32, device=device)
...
class PointPillarsAddZRLater(nn.Module):
    def __init__(self,
                 nclasses=3, 
        ...
        self.backbone = Backbone(#in_channel=64,
                                 ...
                                 out_channels=[66, 132, 264], #MY COMMMENT 66*2= 132, 66*4= 264,
                                 layer_nums=[3, 5, 5])

        self.neck = Neck(#in_channels=[64, 128, 256],
                         in_channels=[66, 132, 264], #MY COMMMENT 66*2= 132, 66*4= 264, 
                         upsample_strides=[1, 2, 4], 
                        #  out_channels=[128, 128, 128])
                         out_channels=[132, 132, 132])

        self.head = Head(#in_channel=384, #MY COMMENT 64*6=384
                         in_channel=396, #MY COMMMENT 66*6= 396

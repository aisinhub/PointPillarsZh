...
class PillarEncoder(nn.Module):
    def __init__(self, voxel_size, point_cloud_range, in_channel, out_channel):
        ...
        self.conv_v2 = nn.Conv1d(13, out_channel, 1, bias=False) # my code
        # self.z_bn = nn.BatchNorm1d(out_channel, eps=1e-3, momentum=0.01)
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar): 
        ...
        # 5. embedding
        po_features = features[:,:,0:4] #(p1 + p2 + ... + pb, num_points, 4)
        pi_features =  features[:,:,4:9] #(p1 + p2 + ... + pb, num_points, 5)
        features = po_features.permute(0, 2, 1).contiguous() # (p1 + p2 + ... + pb, 4, num_points) # my code
        ...
        # z_histの第2，3次元の入れ替え
        p_z_features= torch.cat([pi_features, z_hist],dim=-1)  # (p1 + p2 + ... + pb,1，13)
        p_z_features = p_z_features.view(p_z_features.size(0), p_z_features.size(2), 1) # (p1 + p2 + ... + pb，8，13)
        # z_histを第3次元に沿って num_points に拡張
        p_z_features_expand = p_z_features.expand(-1, -1, num_points) # (p1 + p2 + ... + pb, 8, num_points)

        # 64次元へ拡張
        p_z_features = F.relu(self.bn(self.conv_v2(p_z_features_expand)))  # (p1 + p2 + ... + pb, out_channels, num_points) (P,64,N)
        pooling_p_z_features = torch.max(p_z_features, dim=-1)[0] # (p1 + p2 + ... + pb, out_channels)
...
class PointPillarsLateFusionPFeatureZHist(nn.Module):
    def __init__(self,
                 nclasses=3, 
        ...
        self.head = Head(in_channel=768, n_anchors=2*nclasses, n_classes=nclasses) # 128*3=

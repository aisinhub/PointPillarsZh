...
class PillarEncoder(nn.Module):
    def __init__(self, voxel_size, point_cloud_range, in_channel, out_channel):
        ...
        self.conv_v2 = nn.Conv1d(1, out_channel, 1, bias=False)
        self.bn = nn.BatchNorm1d(out_channel, eps=1e-3, momentum=0.01)
        
        # my 新たな畳み込み層とバッチ正規化層を追加
        self.z_conv = nn.Conv1d(1, out_channel, 1, bias=False)
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar): 
        ...
        # my 新たにzからテンソルを作成
        z_features = pillars[:, :, 2:3].permute(0, 2, 1).contiguous() # (p1 + p2 + ... + pb, 1, num_points)
        z_features = F.relu(self.bn(self.conv_v2(z_features)))  # (p1 + p2 + ... + pb, out_channels, num_points) (P,64,N)
        
        pooling_z_features = torch.max(z_features, dim=-1)[0] # (p1 + p2 + ... + pb, out_channels)
        ...
        for i in range(bs):
            ...
            cur_features_z = pooling_z_features[cur_coors_idx]
            canvas_z = torch.zeros((self.x_l, self.y_l, self.out_channel), dtype=torch.float32, device=device)
            canvas_z[cur_coors[:, 1], cur_coors[:, 2]] = cur_features_z
            canvas_z = canvas_z.permute(2, 1, 0).contiguous()
            batched_z_canvas.append(canvas_z)            
        batched_canvas = torch.stack(batched_canvas, dim=0) # (bs, in_channel, self.y_l, self.x_l)
        batched_z_canvas = torch.stack(batched_z_canvas, dim=0) # (bs, in_channel, self.y_l, self.x_l)

        return batched_canvas, batched_z_canvas
...
class PointPillarsLateFusionZ(nn.Module):
    def __init__(self,
                 nclasses=3, 
        ...
        self.head = Head(in_channel=768, n_anchors=2*nclasses, n_classes=nclasses) # 128*3=384, 384*2=768
    ...
    def forward(self, batched_pts, mode='test', batched_gt_bboxes=None, batched_gt_labels=None):
        ...
        pillar_features, pillar_z_features = self.pillar_encoder(pillars, coors_batch, npoints_per_pillar)

        # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        xs = self.backbone(pillar_features)
        xs_z = self.backbone_v2(pillar_z_features)

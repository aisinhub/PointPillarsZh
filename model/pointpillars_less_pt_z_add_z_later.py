...
# 今回は，後段のBackboneなどを既存手法と同じにしたほうが良いと考え，64+1=65次元の入力をBackboneに入れ，64次元の出力になるようにした
...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar):
        ...
        offset_pt_center = pillars[:, :, :2] - torch.sum(pillars[:, :, :2], dim=1, keepdim=True) / npoints_per_pillar[:, None, None] # (p1 + p2 + ... + pb, num_points, 2)
        ...
        # MY prog ピラーのz重心を計算
        pillars_z_center = torch.mean(pillars[:, :, 2], dim=1) # (p1 + p2 + ... + pb, 1)
        pillars_r_var = torch.var(pillars[:, :, 3], dim=1) # (p1 + p2 + ... + pb, 1)
        # print('--------------- pillars_r_var')
        # print(pillars_r_var.shape)
        
        # 付け足し
        pooling_features = torch.cat([pooling_features, pillars_z_center.unsqueeze(-1)], dim=-1)
        ...
        for i in range(bs):
            ...
            canvas = torch.zeros((self.x_l, self.y_l, 65), dtype=torch.float32, device=device)
...
class PointPillarsLessPtZAddZLater(nn.Module):
    def __init__(self,
                 nclasses=3, 
        ...
        self.backbone = Backbone(#in_channel=64,
                                ...
                                #  out_channels=[65, 130, 195], 
                                 layer_nums=[3, 5, 5])

        self.neck = Neck(in_channels=[64, 128, 256],
                        #  in_channels=[65, 130, 195], 
                         upsample_strides=[1, 2, 4], 
                         out_channels=[128, 128, 128])
                        #  out_channels=[130, 130, 130])

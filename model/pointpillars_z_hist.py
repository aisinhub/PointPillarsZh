# ヒストグラムのための，いろいろな施行 なので，コードは整理されていない
...
########################################### my code >>
        ...
        # print(f"z_hist:{z_hist.size()}")
        
        z_values = pillars[:, :, 2:3]  # (p1 + p2 + ... + pb, num_points, 1)
        ...
        # 各binに含まれる点数を計算
        z_hist = z_mask.sum(dim=1).squeeze(2)  # (p1 + p2 + ... + pb, 8)        
        # print(f"z_hist_sqeeze:{z_hist.size()}")
        

        # print(size(z_hist))
        # print(size(features))
        # end_time = time.time()
        # print(f"3for: {end_time - start_time} ")
        z_hist_expanded = z_hist.expand(-1, pillars.size(1), -1)  # (p1 + p2 + ... + pb, num_points, 8)
        # print(f"z_hist_expand:{z_hist_expanded.size()}")
        
        features = torch.cat([features, z_hist_expanded], dim=-1)  # (p1 + p2 + ... + pb, num_points, 17)
        # features = torch.cat([features, z_hist[:, None, :].expand(-1, pillars.size(1), -1)], dim=-1) # (p1 + p2 + ... + pb, num_points, 17)
...
#         #ビン幅に入るかの真理値のためのマスクz_value_expandの作成
#         z_values = pillars[:, :, 2:3]  # (p1 + p2 + ... + pb, num_points, 1)
#         z_bins_expanded = z_bins.unsqueeze(2)  # (p1 + p2 + ... + pb, 1, 1, 9)
#         z_values_expanded = z_values.unsqueeze(-1)  # (p1 + p2 + ... + pb, num_points, 1, 1)
...
#         # z_valuesが各binに入るかどうかのマスクを計算
#         z_mask = (z_values_expanded >= z_bins_expanded[:, :, :, :-1]) & (z_values_expanded < z_bins_expanded[:, :, :, 1:])  # (p1 + p2 + ... + pb, num_points, 1, 8)
#         # print(f"z_mask:{z_mask.size()}") #>> [35586, 32, 1, 8]
        
#         # 各binに含まれる点数を計算
#         z_hist = z_mask.sum(dim=1).squeeze(2)  # (p1 + p2 + ... + pb, 8, 1)→(p1 + p2 + ... + pb, 8)
#         # print(f"z_hist_sqeeze:{z_hist.size()}") #>> [35586, 1, 8]
        
#         # 各ピラー内のx, yが0でない点の個数をカウント        
#         non_zero_count = ((pillars[:, :, 0] != 0) & (pillars[:, :, 1] != 0)).sum(dim=1, keepdim=True)  # (p1 + p2 + ... + pb, 1)
#         # print(f"z_hist_count:{non_zero_count.size()}") #>> [35586, 1]
        
#         #計算の際，型を揃えるため，カウント行列の次元を拡張
#         non_zero_count_expanded=non_zero_count.expand(-1, 8).unsqueeze(1)
#         # print(f"z_hist_count_expand:{non_zero_count_expanded.size()}") #>> [35586, 1, 8]
        
        
#         # 点の個数でhistを正規化
#         z_hist = z_hist / non_zero_count_expanded.float()  # (p1 + p2 + ... + pb, 1,8)
#         # print(f"z_hist_norm:{z_hist.size()}") #>> [35586, 1, 8]
...
#         #featureと結合
#         features = torch.cat([features, z_hist_expanded], dim=-1)  # (p1 + p2 + ... + pb, num_points, 12)
#         # features = torch.cat([features, z_hist[:, None, :].expand(-1, pillars.size(1), -1)], dim=-1) # (p1 + p2 + ... + pb, num_points, 17)
...
class PointPillarsZHist(nn.Module):
    def __init__(self,
                 nclasses=3, 

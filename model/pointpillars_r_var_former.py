...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar):
        ...
        pillars_r_var = torch.var(pillars[:, :, 3], dim=1).unsqueeze(-1).unsqueeze(-1).expand(p_dim, n_dim, 1)
        
        # tic()       
        # コンキャット
        features = torch.cat([pillars, offset_pt_center, x_offset_pi_center, y_offset_pi_center, pillars_r_var], dim=-1) # (p1 + p2 + ... + pb, num_points, 10)
        # toc()
...
class PointPillarsRVarFormer(nn.Module):
    def __init__(self,
                 nclasses=3, 

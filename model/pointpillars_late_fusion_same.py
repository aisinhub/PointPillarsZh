...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar): 
        ...
        pooling_features_v2 = torch.max(features, dim=-1)[0] # (p1 + p2 + ... + pb, out_channels)
        ...
        for i in range(bs):
            ...
            cur_features_z_hist = pooling_features_v2[cur_coors_idx]
...
class PointPillarsLateFusionSame(nn.Module):
    def __init__(self,
                 nclasses=3, 
    ...
    def forward(self, batched_pts, mode='test', batched_gt_bboxes=None, batched_gt_labels=None):
        ...
        pillar_features, pillar_features_v2 = self.pillar_encoder(pillars, coors_batch, npoints_per_pillar)

        # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)]
        xs = self.backbone(pillar_features)
        xs_z = self.backbone_v2(pillar_features_v2)

...
class PillarEncoder(nn.Module):
    def __init__(self, voxel_size, point_cloud_range, in_channel, out_channel):
        ...
        self.conv_v2 = nn.Conv1d(5, out_channel, 1, bias=False) # my code
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar): 
        ...
        ############################ ↓ my code
        pi_features = pi_features.permute(0, 2, 1).contiguous() # (p1 + p2 + ... + pb, 4, num_points) # my code
        pi_features = F.relu(self.bn_v2(self.conv_v2(pi_features)))  # (p1 + p2 + ... + pb, out_channels, num_points)   PointNet使っている部分3つのやつ(conv, bn,relu)で
        
        pooling_z_hist_features = torch.max(pi_features, dim=-1)[0] # (p1 + p2 + ... + pb, out_channels)
...
class PointPillarsLateFusionPFeature(nn.Module):
    def __init__(self,
                 nclasses=3, 

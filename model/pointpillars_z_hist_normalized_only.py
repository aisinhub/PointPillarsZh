...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar):
        ...
        # 3. encoder
        features_temp = torch.cat([pillars, offset_pt_center, x_offset_pi_center, y_offset_pi_center], dim=-1) # (p1 + p2 + ... + pb, num_points, 9)
        features_temp[:, :, 0:1] = x_offset_pi_center # tmp
        features_temp[:, :, 1:2] = y_offset_pi_center # tmp
        ...
        # npoints_per_pillar = torch.randint(1, num_points, num_pillars)  # 各ピラー内のポイント数
        ...
        # print(f"z_hist:{z_hist.size()}") #>> [35586, 8]
        ...
        #z_histを[35586, 1, 8]→[35586, 32, 1]に次元拡張
        z_hist_expanded = z_hist.expand(-1, pillars.size(1), -1)  # [35586, 1, 8]→(p1 + p2 + ... + pb, num_points, 8)
        # print(f"z_hist_expand:{z_hist_expanded.size()}") #>> [35586, 32, 8]
        
        features = z_hist_expanded
        #featureと結合
        # features = torch.cat([features, z_hist_expanded], dim=-1)  # (p1 + p2 + ... + pb, num_points, 17)
...
################################################## << my code
        ...
        # 5. embedding
        features = features.permute(0, 2, 1).contiguous() # (p1 + p2 + ... + pb, 17, num_points)
        features = F.relu(self.bn(self.conv(features)))  # (p1 + p2 + ... + pb, 17, num_points)→(p1 + p2 + ... + pb, out_channels, num_points)   PointNet使っている部分3つのやつ(conv, bn,relu)で
...
class PointPillarsZHistNormalizedOnly(nn.Module):
    def __init__(self,
                 nclasses=3, 

...
def calculate_normals(grouped_tensor, k):
    # # GPU上のテンソルをCPU上にコピーしてからNumpyに変換する
    # grouped_cpu = grouped_tensor.cpu()  # GPU上からCPU上へのコピー
    
    # # GPU上のテンソルをCPUに移動させる
    # grouped_cpu = grouped_tensor.cpu()

    # GPU上のテンソルをCPUに移動させ、それからNumpy配列に変換する
    xyz_np = grouped_tensor.detach().cpu().view(-1, grouped_tensor.shape[-1]).numpy()
    
    # # PyTorchからNumpyに変換
    # xyz_np = grouped_tensor.view(-1, grouped_tensor.shape[-1]).numpy()
    ...
    for i in range(len(xyz_np)):
        ...
        # 近傍点の座標を取得
        neighbors = xyz_np[neighbor_indices]

        # 各次元の分散を計算
        variances = np.var(neighbors, axis=0)

        # 分散が非常に小さい場合（同一平面上にあると判断）、法線をゼロベクトルとする
        if np.any(variances < 1e-6):
            normal = np.zeros(3)
        else:
            # PCAを使って平面を近似し、法線ベクトルを求める
            pca = PCA(n_components=3)
            pca.fit(neighbors)
            normal = pca.components_[2]  # 最後の主成分が法線を表す

        # 法線ベクトルをリストに追加
        normals.append(normal)

    # 推定された法線(リスト)をNumPyの配列に変換し、PyTorchのテンソルにreshape
    normals_np = np.array(normals)
    normals_tensor = torch.from_numpy(normals_np)
    
    ##GPU上のテンソルに変換 
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    pytorch_tensor_gpu = normals_tensor.to(device)
    
    pytorch_tensor_gpu_reshape =pytorch_tensor_gpu.reshape(grouped_tensor.size())

    return pytorch_tensor_gpu_reshape
...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar):
        ...
        # 3. encoder
        # My prog 単純な法線推定
        xyz_points = pillars[:, :, :3]
        normal = calculate_normals(xyz_points, 5)
                

        # コンキャット
        features = torch.cat([pillars, offset_pt_center, x_offset_pi_center, y_offset_pi_center, normal], dim=-1) # (p1 + p2 + ... + pb, num_points, 12)
...
class PointPillarsNormalFormer(nn.Module):
    def __init__(self,
                 nclasses=3, 
        ...
        self.head = Head(in_channel=384, #MY COMMENT 64*6=384
                        #  in_channel=390, #MY COMMMENT 65*6= 390
                         n_anchors=2*nclasses, n_classes=nclasses)

...
# 追加部分: z軸方向に8分割し、各z幅に含まれる点数を計算
        ...
        # toc("before 3for")
        end_time = time.time()
        print(f"before 3for: {end_time - start_time} ")
        start_time = time.time()
        # tic()
        for i in range(num_pillars):
            for j in range(num_points):
                for k in range(7):
                    # z_hist[:, i] = torch.sum((pillars[:, :, 2:3] >= z_bins[:, :, i:i+1]) & (pillars[:, :, 2:3] < z_bins[:, :, i+1]), dim=1).squeeze()
                    # z_hist[:, i] = torch.sum((pillars[:, :, 2:3] >= z_bins[:, :, i]) & (pillars[:, :, 2:3] < z_bins[:, :, i+1]), dim=1).squeeze()
                    if (pillars[i, j, 2:3] >= z_bins[i, 0, k]) and (pillars[i, j, 2:3] < z_bins[i, 0, k+1]):
                        z_hist[i, k] += 1
        
        z_hist = z_hist / npoints_per_pillar[:, None].float() # 正規化
        # z_hist_3d = z_hist.unsqueeze(1)
        # z_hist_

...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar):
        ...
        # # MY prog~
        # pillars_center = torch.sum(pillars[:, :, :3], dim=1, keepdim=True) / npoints_per_pillar[:, None, None] # (p1 + p2 + ... + pb, num_points, 3)
        # pillars_center_ext = pillars_center[:,0,:]
        # #~ My prog
        
        # MY prog~
        pillars_center = torch.mean(pillars[:, :, :3], dim=1) # (p1 + p2 + ... + pb, num_points, 3)
        #~ My prog
        ...
        features = torch.cat([pillars, x_offset_pi_center, y_offset_pi_center], dim=-1) # (p1 + p2 + ... + pb, num_points, 9)
        ...
        #MY Prog
        pooling_features = torch.cat([pooling_features, pillars_center], dim=-1)
        ...
        for i in range(bs):
            ...
            canvas = torch.zeros((self.x_l, self.y_l, 64), dtype=torch.float32, device=device)
...
class PointPillarsLaterCenterXyz64ch(nn.Module):
    def __init__(self,
                 nclasses=3, 
        ...
        self.head = Head(#in_channel=384, #MY COMMENT 64*6=384
                         in_channel=384, #MY COMMMENT 67*6= 402 erro zl 384

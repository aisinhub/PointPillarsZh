...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar):
        ...
        #5.5 MY Prog
        pooling_features = torch.cat([pooling_features, pillars_center_ext], dim=-1)
        ...
        for i in range(bs):
            ...
            canvas = torch.zeros((self.x_l, self.y_l, 67), dtype=torch.float32, device=device)
...
class PointPillarsLaterCenterXyz(nn.Module):
    def __init__(self,
                 nclasses=3, 
        ...
        self.backbone = Backbone(#in_channel=64,
                                 ...
                                 out_channels=[67, 134, 268], 
                                 layer_nums=[3, 5, 5])

        self.neck = Neck(#in_channels=[64, 128, 256],
                         in_channels=[67, 134, 268], 
                         upsample_strides=[1, 2, 4], 
                        #  out_channels=[128, 128, 128])
                         out_channels=[134, 134, 134])

        self.head = Head(#in_channel=384, #MY COMMENT 64*6=384
                         in_channel=402, #MY COMMMENT 67*6= 402

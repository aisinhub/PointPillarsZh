...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar):
        ...
        bin_z=2
        histograms = []
        for i in range(pillars.size(0)):
            x_values = pillars[i, :, 0] #x_valuenには，pillar[i,1,0]~[i:N,0]のN個のデータが入る
            z_values = pillars[i, :, 2]
            x_min, x_max = torch.min(x_values), torch.max(x_values)
            z_min, z_max = torch.min(z_values), torch.max(z_values)

            # x軸を4分割、z軸を8分割
            x_bins = torch.linspace(x_min, x_max, bin_x+1).to(device)
            z_bins = torch.linspace(z_min, z_max, bin_z+1).to(device)

            # ヒストグラムを作成（32ビン）
            histogram = torch.zeros((bin_x, bin_z), device=device)

            # 各点を対応するビンに割り当て
            # bucketizeは，ビンのエッジに正確に一致する値については、その次のビンに含めることがあるため，この処理が必要 
            for x, z in zip(x_values, z_values):
                x_idx = torch.bucketize(x, x_bins) - 1 # torch.bucketizeを使って、xおよびzの値がどのビンに属するかを決定する
                z_idx = torch.bucketize(z, z_bins) - 1

                # ビンのインデックスが範囲外にならないように調整
                if x_idx == bin_x: #もし，インデックスx_idxが最大値(bin_x)のときは， x_idx-1してやる．これをしないと，状況によっては範囲外になるらしい?ため
                    x_idx = bin_x -1  # xの最大値が最後のビンに含まれるように調整
                if z_idx == bin_z:
                    z_idx = bin_z -1  # zの最大値が最後のビンに含まれるように調整

                histogram[x_idx, z_idx] += 1 #点群の各点が属するビンを特定し、そのビンのカウントをインクリメント

            # ヒストグラムをリストに追加
            histograms.append(histogram.view(-1))

        # ヒストグラムをテンソルに変換し、featuresにコンキャット
        histograms = torch.stack(histograms)
        histograms = histograms.unsqueeze(1).expand(-1, pillars.size(1), -1)  # (p1 + p2 + ... + pb, num_points, 36)
        features = torch.cat([features, histograms], dim=-1)  # (p1 + p2 + ... + pb, num_points, 9 + 4=13)
...
class PointPillarsXZHist(nn.Module):
    def __init__(self,
                 nclasses=3, 
